...
const premium = JSON.parse(fs.readFileSync('./assets/database/data/role/premium.json'))
...
const addPremiumUser = (userId, expired, _dir) => {
	...
	fs.writeFileSync("../assets/database/data/role/premium.json", JSON.stringify(_dir));
};
...
const expiredPremiumCheck = (XeonBotInc, msg, _dir) => {
	setInterval(() => {
		...
		if (position !== null) {
			...
			fs.writeFileSync("../assets/database/data/role/premium.json", JSON.stringify(_dir));
			idny ? XeonBotInc.sendMessage(idny, { text: "Premium Anda sudah habis, silakan beli lagi." }) : "";
			idny = false;
		}
	}, 1000);
};
